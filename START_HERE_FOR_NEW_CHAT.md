...
- Текущая стадия:
  - рабочий путь локального портала уже переведён на системный браузер и подтверждён на живом телефоне;
  ...
  - подтверждено, что `HotspotPortalActivity` теперь не выбрасывает телефон из сети `GOSHA-A-*`;
  - подтверждено, что после нажатия `Подключить робота` приложение открывает `http://192.168.4.1` во внешнем браузере;
  - подтверждено, что по возврату назад приложение снова показывает экран ожидания завершения настройки;
  - следующий обязательный шаг теперь не диагностика белого экрана, а живой проход `scan` / выбор домашнего `Wi-Fi` / `submit` / возврат робота в основной сценарий.
- Что именно проверять в следующем чате:
  - загружается ли страница робота в системном браузере без ручных обходов;

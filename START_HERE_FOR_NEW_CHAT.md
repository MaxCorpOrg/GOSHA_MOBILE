...
- Текущая стадия:
  - первая кодовая правка уже сделана и повторно проверена на живом телефоне;
  ...
  - следующий обязательный шаг теперь не анализ Android, а проверка ответа самого робота по `192.168.4.1`.
- Что именно проверять в следующем чате:
  - отвечает ли робот на `ping 192.168.4.1`;
  - слушает ли робот локальный HTTP на `80` и `WebSocket` на `8080`;
  - работают ли `scan`, `submit`, `exit`, если HTTP начнёт отвечать;

...
- В этой ветке уже сделана первая прикладная правка для `HotspotPortalActivity`:
  - HTML портала загружается через `RobotPortalClient`;
  - внутренние `fetch`-запросы страницы перекинуты через `RobotPortalBridge`;
  - добавлены диагностические сообщения `HotspotPortal`.
- `2026-05-27` подтверждено на живом телефоне:
  ...
  - `HotspotPortalActivity` после нажатия `Подключить робота` действительно открывается;
...
- Следующий живой вывод после этого фикса:
  - проблема уже не в потере сети Android;
  - телефон на `GOSHA-A-1BE1` не получает ответ от `192.168.4.1`;
  - `ping 192.168.4.1` не отвечает;
  - `nc` в `192.168.4.1:80` и `192.168.4.1:8080` уходит в таймаут.
...
3. Пройти живой сценарий подключения робота и проверить:
   - отвечает ли `192.168.4.1` вообще;
   - слушают ли локальные порты `80` и `8080`;
   - только после этого повторять `scan`, `submit` и возврат в приложение.
4. Если проблема останется, работать уже не только с Android, а и с локальным контуром робота:
   - `HotspotPortal`
   - `GoshaRobotWifi`
   - `ping 192.168.4.1`
   - `nc -w 3 192.168.4.1 80`
   - `nc -w 3 192.168.4.1 8080`
5. После восстановления ответа от робота повторить живой проход целиком и затем закоммитить итоговую рабочую точку портала в эту же ветку.
6. Для iOS:

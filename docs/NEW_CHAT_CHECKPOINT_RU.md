...
- Следующий живой шаг после этой серии правок:
  ...
  - пройти регистрацию и локальный портал до полного `submit`;
  - дождаться возврата в `MENU`;
  - удержать приложение в этом состоянии не меньше 60 секунд и убедиться, что оно не выбрасывает пользователя назад в шаг переподключения;
  - дополнительно подтвердить на панели:
    - появляется свежий `mobile_presence`;
    - при локальном обнаружении робота виден `local_host`;
...
3. Пройти живой сценарий подключения робота и проверить:
   ...
   - возвращается ли приложение в `MENU`;
...
5. После успешного живого прогона отдельно проверить панель:

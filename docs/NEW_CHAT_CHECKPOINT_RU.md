...
3. Пройти живой сценарий подключения робота и проверить:
   ...
   - работает ли выбор сети;
   - проходит ли `submit`;
   - возвращается ли приложение после настройки `Wi‑Fi`.
4. Если проблема останется, снять живые логи:
   - `HotspotPortal`
   - `GoshaRobotWifi`
5. После подтверждения живого прохода закоммитить итоговую рабочую точку портала в эту же ветку.
6. Для iOS:

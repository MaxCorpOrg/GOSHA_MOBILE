...
3. Текущий главный мобильный блокер уже смещён:
   - `2026-05-27` подтверждено на живом телефоне, что после нажатия `Подключить робота` приложение открывает `192.168.4.1` во внешнем `Chrome`
   - подтверждено, что страница робота реально загружается
   - подтверждено, что по возврату назад приложение снова показывает экран ожидания
   - незакрытый хвост теперь не в открытии портала, а в полном живом проходе `scan` / `submit` / возврат робота в основной сценарий
...
5. В этой ветке уже сделана рабочая прикладная правка:
   - локальный портал открывается через системный браузер;
   - `HotspotPortalActivity` оставлен как запасной путь, если внешний браузер не откроется;
   - в запасном экране добавлены диагностические сообщения `HotspotPortal`;
...
6. Следующий приоритет:
   - пройти живой сценарий уже до выбора домашней сети и `submit`;
   - проверить, что робот после этого уходит из `GOSHA-*`, а приложение доводит процесс до возврата;
   - если именно в системном браузере появится новый отказ, снова собрать живые логи и только потом решать, нужен ли возврат к прошивке;
   - сохранять живые логи `HotspotPortal` и `GoshaRobotWifi`

...
6. Следующий приоритет:
   ...
   - затем снова перевести робота в режим подключения новой точки доступа и пройти полный живой путь заново;
   - проверить и ускорить именно пользовательские задержки:
     - долгое нажатие `Подключить`;
     - долгое определение статуса робота на `MENU`;
     - честные тексты загрузки в шаге повторного `Wi‑Fi`;
   - после успешного возврата телефона в домашний `Wi‑Fi` отдельно подтвердить:
     - стартует ли `ConnectorForegroundService`;
     - повторяется ли после нового полного цикла уже подтверждённая долгая живость `mobile_presence` после сворачивания;
     - сохраняется ли сигнал панели дольше 3 минут;

...
6. Следующий приоритет:
   - повторить живой проход на телефоне именно с веткой `feature/mobile-hotspot-portal`
   - если портал открылся, довести сценарий до возврата в приложение
   - если проблема останется, снять живые логи `HotspotPortal` и `GoshaRobotWifi`

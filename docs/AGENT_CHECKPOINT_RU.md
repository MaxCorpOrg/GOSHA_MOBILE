...
6. Следующий приоритет:
   - пройти живой сценарий уже до выбора домашней сети, `submit` и возврата в `MENU`;
   - удержать это состояние не меньше 60 секунд и убедиться, что приложение не открывает шаг переподключения само;
   - проверить, что робот после этого уходит из `GOSHA-*`, а приложение остаётся в рабочем меню;

...
3. Текущий главный блокер теперь не в приложении, а на стороне локального портала робота:
   - `2026-05-27` подтверждено на живом телефоне, что `HotspotPortalActivity` открывается и телефон остаётся в `GOSHA-A-1BE1`
   - но `192.168.4.1` не отвечает даже при прямой проверке из телефона
...
5. В этой ветке уже сделана первая прикладная правка:
   - HTML локального портала загружается через `RobotPortalClient`;
...
6. Следующий приоритет:
   - проверять уже не потерю сети Android, а реальную доступность `192.168.4.1` со стороны робота;
   - если робот начнёт отвечать, довести сценарий до возврата в приложение;
   - если ответа так и не будет, переходить в `GOSHA_FIRMWARE` / `fw-portal` и разбирать локальный HTTP-контур на роботе;

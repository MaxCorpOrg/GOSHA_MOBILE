...
- В этой ветке уже сделана первая целевая правка:
  - HTML локального портала теперь загружается через `RobotPortalClient`, а не напрямую через сетевой слой `WebView`;
  - внутренние запросы страницы (`fetch`) тоже перекинуты через `RobotPortalBridge` и тот же `RobotPortalClient`;
  - это убирает прямую зависимость от того, сможет ли конкретный Android `WebView` честно открыть `192.168.4.1` на сети робота.
...
- После этого живой прогон показал уже следующий, более глубокий блокер:
  - даже при активной сети `GOSHA-A-1BE1` локальный портал робота не отвечает;
  - `HotspotPortal` пишет `code=0 bytes=0`;
  - `ping 192.168.4.1` не отвечает;
  - `nc -w 3 192.168.4.1 80` и `192.168.4.1 8080` уходят в таймаут;
  - значит текущий хвост сместился из Android-клиента в сторону локального HTTP/`WebSocket` контура робота.
...
1. Перейти в парную диагностику `GOSHA_MOBILE` + `GOSHA_FIRMWARE` для локального портала:
   - Android уже подтверждён как удерживающий сеть робота;
   - теперь нужно поднять ответ самого робота на `192.168.4.1`.
2. На стороне робота проверить, почему нет ответа на:
   - `ping 192.168.4.1`
   - HTTP `80`
   - `WebSocket` `8080`
3. После восстановления ответа от робота повторить живой проход Android целиком:

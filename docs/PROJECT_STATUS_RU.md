...
- Из-за этого пока не подтверждено:
  - завершение настройки робота через локальную страницу с реальным домашним паролем
  - автоматический уход робота из `GOSHA-*` после `submit`
...
1. Продолжить локальный контур `GOSHA_MOBILE` для портала:
   ...
   - следующий шаг теперь не поднимать ответ робота, а довести живой сценарий до `submit` и возврата.
2. Повторить живой проход Android целиком:
   - ввод кода подключения
   - поиск `GOSHA-*`
   - рабочее открытие `192.168.4.1` во встроенном `HotspotPortalActivity`
   - `scan`
   - `submit`
   - переход в главное меню без ручного ввода адреса
3. Затем отдельно дополировать тексты и переходы шага `runtime`:
...
4. Если уже во встроенном портале появится новый отказ, только тогда снова проверять парно:
   - `HotspotPortal`
   - `GoshaRobotWifi`
   - `RobotPortalClient`
   - сам встроенный `HotspotPortalActivity`
   - и только после этого решать, нужен ли возврат к диагностике `GOSHA_FIRMWARE`.
5. После этого зафиксировать первую полностью рабочую контрольную точку `GOSHA_MOBILE`.
6. Для iOS:

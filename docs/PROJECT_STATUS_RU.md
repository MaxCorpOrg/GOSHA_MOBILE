...
1. Повторить живой прогон Android на реальном телефоне уже с новой веткой `feature/mobile-hotspot-portal`:
   - ввод кода подключения
   - поиск `GOSHA-*`
   - переходный поиск `Xiaozhi-*`
   - рабочее открытие `192.168.4.1`
   - переход в главное меню без ручного ввода адреса
2. Если пустой экран исчез, зафиксировать эту точку как первый рабочий мобильный проход от точки доступа до возврата в приложение.
3. Если останется сбой, снять живые логи `HotspotPortal` и отдельно проверить:
   - уходит ли запрос `GET /`;
   - проходят ли `fetch /scan`, `fetch /submit`, `fetch /exit`;
   - отвечает ли локальный `WebSocket` `ws://192.168.4.1:8080/ws`.
4. Потом отдельно дополировать тексты и переходы шага `runtime`:

...
- Тем же днём уточнено и следующее:
  ...
  - реальная проблема была не в роботе, а во встроенном `WebView` на этом устройстве;
  - как основной пользовательский путь локальный портал переведён на системный браузер через `Intent.ACTION_VIEW`.
- Финальная проверка на телефоне `TECNO LI9` уже подтверждила:
  - после нажатия `Подключить робота` приложение открывает `Chrome` с адресом `192.168.4.1`;
  - страница робота загружается и показывает список доступных сетей;
  - по системной кнопке назад приложение возвращается в `MainActivity`;
  - после возврата показывается корректный экран ожидания, пока телефон ещё находится в сети робота.
...
1. Продолжить локальный контур `GOSHA_MOBILE` для портала:
   - основной Android-путь открытия портала уже подтверждён;
...
2. Повторить живой проход Android целиком:
   ...
   - поиск `GOSHA-*`
   - рабочее открытие `192.168.4.1` во внешнем браузере
   - `scan`
   - `submit`
...
4. Если уже в системном браузере появится новый отказ, только тогда снова проверять парно:
   - `HotspotPortal`
   - `GoshaRobotWifi`
   - встроенный запасной `HotspotPortalActivity`

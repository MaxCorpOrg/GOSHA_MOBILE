...
- Живое состояние приложения очищено от старого локального адреса. Сохранённый `robot_id=gosha-main` не считается достаточным доказательством физической личности устройства.
- Добавлен локальный read-only запрос `gosha.identity.get` и проверка ответа `gosha.identity.result`; общий поиск по подсети разрешён только с непустым ожидаемым `device_id`, а найденный адрес сохраняется только после совпадения личности.
- `device_id` из пакета подключения сохраняется отдельно от пользовательского `robot_id`; значения аппаратной личности не выводятся в документы или журнал.
- Полный прогон `testClientDebugUnitTest`, `assembleClientDebug` и `lintClientDebug` завершён успешно.
- Этот кандидат ещё не установлен на телефон и не проходил новую живую приёмку. Ранее установленный APK и его контрольная сумма относятся к предыдущей принятой версии.
...
- `2026-07-23` исправлена семантика задачи восстановления: возврат самой сети больше не закрывает `wifi_recovery`; состояние остаётся `running` до прямого подтверждения локального робота.
- Добавлены отдельные события повторной проверки, успешного локального подтверждения и терминального `timed_out`.
- `timed_out` закреплён как конечное состояние: позднее обнаружение обновляет связность, но не переписывает историю старой задачи без нового `correlation_id`.
- Активная задача сама запускает ограниченные повторы и при отсутствии сигнала панели, поэтому состояние не остаётся `running` бессрочно, если робот одновременно недоступен локально и через платформу.
- После исчерпания повторов экран больше не обещает продолжающуюся проверку, а показывает конечное действие: обновить статус или сменить Wi-Fi робота.
- Живой Android P1 на `TECNO LI9` повторно закрыт уже на точном текущем APK: приложение в одном процессе обнаруживает потерю и возврат домашнего Wi-Fi, честно меняет карточку и восстанавливает фоновый коннектор.
...
- `2026-08-24` внесён P1-патч Android после регистрации нового робота:
  - `edge_hub_url` из пакета подключения читается отдельно от `mobile_profile.mcp_endpoint_base` и `cloud_endpoint`; MCP-адрес больше не подставляется как Hub по умолчанию;
...
- `2026-07-23` повторно закрыт полный Android P1 на точной итоговой сборке:
  - возврат Wi-Fi сам по себе сохраняет задачу восстановления в `running`;
  - завершение публикуется только после успешной локальной проверки робота;
  - тот же процесс и фоновая служба пережили цикл отключения и восстановления;
  - все три связи в снимке платформы имеют состояние `available`, активных задач нет;
  - текущий APK установлен на устройство, его SHA-256 указан выше.
